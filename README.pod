...
When creating objects with two (or zero) filehandles, it is assumed you want to fork the $passer object and afterwards you are supposed to fork and then call either $passer->i_am_parent or $passer->i_am_child. If you don't plan on forking and instead wish to establish the passing connection via the filesystem, you should only pass one filehandle in.
